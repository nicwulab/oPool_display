import pandas as pd
# Configurable output name - can be set from UI
output_name = "library"  # Default name, can be overridden
import argparse
import sys
import numpy as np
import os
...
parser = argparse.ArgumentParser(description="Generate final antibody library with configurable output name")
parser.add_argument("-o", "--output", default="library", help="Output name prefix (default: library)")
...
TB = pd.read_csv('HA_screen/data/TableS1.csv', header = 1)
TB_seg = pd.DataFrame(Result)
TB = TB[TB.Name.isin(TB_seg.Name)]

# for storing the result
pd.merge(TB, TB_seg).to_csv(f'ui_results/{output_name}_oPool_design.csv')
...
final_sequences = []
pool_sequences = {}  # Dictionary to store sequences by pool

# Process each segment
for i in range(len(TB_seg)):
    antibody_name = TB_seg.Name.iloc[i]
    group = TB_seg.group.iloc[i]
    
    # Extract pool name from group (e.g., "Re_assembled_0_Sim_.65.fa" -> "Pool_0")
    pool_name = group.replace('Re_assembled_', 'Pool_').replace('_Sim_.65.fa', '')
    
    # Seg1: [up_seqeunce][Seg1]
    seg1_name = f"{antibody_name}_Seg1"
    seg1_seq = up_seqeunce + TB_seg.Seg1.iloc[i]
    final_sequences.append([seg1_name, seg1_seq, pool_name])
    
    # Seg2: reverse_complement([Overlap1][Seg2][Overlap2])
    seg2_name = f"{antibody_name}_Seg2"
    seg2_seq = str(Seq(TB_seg.Seg2.iloc[i]).reverse_complement())
    final_sequences.append([seg2_name, seg2_seq, pool_name])
    
    # Seg3: [Overlap2][Seg3][Overlap3]
    seg3_name = f"{antibody_name}_Seg3"
    seg3_seq = TB_seg.Seg3.iloc[i]
    final_sequences.append([seg3_name, seg3_seq, pool_name])
    
    # Seg4: reverse_complement([Overlap3][Seg4])
    seg4_name = f"{antibody_name}_Seg4"
    seg4_seq = str(Seq(TB_seg.Seg4.iloc[i]).reverse_complement())
    final_sequences.append([seg4_name, seg4_seq, pool_name])
    
    # Store sequences by pool for separate FASTA files
    if pool_name not in pool_sequences:
        pool_sequences[pool_name] = []
    pool_sequences[pool_name].extend([
        [seg1_name, seg1_seq],
        [seg2_name, seg2_seq],
        [seg3_name, seg3_seq],
        [seg4_name, seg4_seq]
...
# Create DataFrame for final sequences with pool column
final_df = pd.DataFrame(final_sequences, columns=['name', 'sequence', 'pool'])

# Save as TSV with pool column
tsv_filename = f"ui_results/{output_name}_final_sequences.tsv"
final_df.to_csv(tsv_filename, sep='\t', index=False)
print(f"TSV file created: {tsv_filename}")

# Create separate FASTA files for each pool
for pool_name, sequences in pool_sequences.items():
    fasta_filename = f"ui_results/{output_name}_{pool_name}_sequences.fasta"
    with open(fasta_filename, 'w') as f:
        for name, seq in sequences:
            f.write(f">{name}\n{seq}\n")
    print(f"FASTA file created: {fasta_filename} ({len(sequences)} sequences)")

print(f"Total sequences: {len(final_sequences)}")
print(f"Antibodies processed: {len(TB_seg)}")
print(f"Pools created: {len(pool_sequences)}")
