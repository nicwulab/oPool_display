...
def process_seq(args):
    index, vh_seq, vl_seq, name, r_val = args
    Seq = vl_seq + "GGGGS" * 3 + vh_seq
    temp_result = [AA2DNA_random(Seq) for _ in range(r_val)]
    local_result2 = []
    for ii, seq in enumerate(temp_result):
        Trunk = len(seq) // 99 + (len(seq) % 99 != 0)
        for N in range(Trunk):
            fragment = seq[N * 99:(N + 1) * 99]
            local_result2.append([f"{name}:{N}-{ii}", fragment])
    return local_result2

# Initialize codon tables globally
C_Table_R = {}
...
if __name__ == '__main__':
        ...
        with Progress() as progress:
            ...
            args_list = [(i, TB.VH_AA.iloc[i], TB.VL_AA.iloc[i], TB.Name.iloc[i], R) for i in range(len(TB))]
            ...
            with Pool(processes=num_processes, initializer=init_worker) as pool:
                ...
                for i, result in enumerate(pool.imap_unordered(process_seq, args_list)):
