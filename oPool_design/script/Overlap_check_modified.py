...
TB_VL = pd.read_csv('HA_screen/result/VL.tsv.gz', sep ='\t')
...
def Blast_check():
    ...
    while len(TMP_tmp) != 25 *3:
        i += 1
        os.system(f"blastn -query Primer/{group}  -db blastDB/{group}  -outfmt '6 qacc sacc evalue pident qcovs' -evalue 1e-1 -num_threads 8 -max_hsps 2 -word_size {i} > ui_results/blast/{group}")
        os.system("cat ui_results/blast/" + group + "| awk '{print $1}'| sort| uniq -c| awk '{print $1,$2}'| grep '^1 ' >  ui_results/segs_id/" + group + '.list')
        with open(f"ui_results/segs_id/{group}.list", 'r') as F:
            TMP = F.read().split('\n')[:-1]
    ...
    TMP_tmp.to_csv(f'ui_results/segs_id/{group}.csv')


# read the table

Group = [i for i in os.listdir("ui_results") if i.startswith("Re_assembled")]
TB = pd.read_csv("HA_screen/data/TableS1.csv", header=1)
AA_L = 10
os.system('mkdir Primer')
os.system('mkdir blastDB')
os.system('mkdir ui_results/blast')
os.system('mkdir ui_results/segs_id')

for group in Group:
    Fasta_DB = "ui_results/" + group
    ...
    F.close()
    os.system(f"makeblastdb -in ui_results/{group} -dbtype nucl -parse_seqids -out blastDB/{group}")
    Blast_check()
